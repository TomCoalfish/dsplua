...
namespace filesys 
{
    ...
    struct DirEntry 
    {
        ...
        DirEntry() {}
        DirEntry(filesystem::directory_entry & e) : entry(e) {} 
    ...
    struct FileStatus
    {
        ...
        FileStatus(filesystem::file_status s) : status(s) {} 
    ...
    struct FileSystem
    {            
        std::string absolute(const std::string& p) { return filesystem::absolute(p).string(); }
        std::string canonical(const std::string& p) { return filesystem::canonical(p).string(); }
        std::string weakly_canonical(const std::string& p) { return filesystem::weakly_canonical(p).string(); }
        std::string relative(const std::string& p) { return filesystem::relative(p).string(); }
        std::string proximate(const std::string & p) { return filesystem::proximate(p).string(); }
        
        
        void copy(const std::string & from, const std::string & to) { filesystem::copy(from, to); }
        void copy_file(const std::string & from, const std::string & to) { filesystem::copy_file(from, to); }
        void copy_symlink(const std::string & from, const std::string & to) { filesystem::copy_symlink(from, to); }
        
        bool create_directory(const std::string & path) { return filesystem::create_directory(path); }
        bool create_directories(const std::string & path) { return filesystem::create_directories(path); }

        void create_hard_link(const std::string & target, const std::string & link) { filesystem::create_hard_link(target,link); }
        void create_symlink(const std::string & target, const std::string & link) { filesystem::create_symlink(target,link); }
        void create_directory_symlink(const std::string & target, const std::string & link) { filesystem::create_directory_symlink(target,link); }

        std::string current_path() { return filesystem::current_path(); }
        bool exists(const std::string & p) { return filesystem::exists(p); }
        bool equivalent(const std::string & p1, const std::string & p2) { return filesystem::equivalent(p1,p2); }

        uintmax_t file_size(const std::string & p) { return filesystem::file_size(p); }
        uintmax_t hard_link_count(const std::string & p) { return filesystem::hard_link_count(p); }
        ...
        */
        std::string read_symlink(const std::string & p) { return filesystem::read_symlink(p).string(); }

        bool remove(const std::string & p) { return filesystem::remove(p); }
        uintmax_t remove_all(const std::string & p) { return filesystem::remove_all(p); }        
        void rename(const std::string & old, const std::string &newf) { filesystem::rename(old,newf); }

        void resize_file(const std::string &p, uintmax_t size) { filesystem::resize_file(p, size); }
        std::string temp_directory_path() { return filesystem::temp_directory_path().string(); }

        bool is_block_file(const std::string &p) { return filesystem::is_block_file(p); }
        bool is_character_file(const std::string &p) { return filesystem::is_character_file(p); }
        bool is_directory(const std::string &p) { return filesystem::is_directory(p); }
        bool is_empty(const std::string &p) { return filesystem::is_empty(p); }
        bool is_fifo(const std::string &p) { return filesystem::is_fifo(p); }
        bool is_other(const std::string &p) { return filesystem::is_other(p); }
        bool is_regular_file(const std::string &p) { return filesystem::is_regular_file(p); }
        bool is_socket(const std::string &p) { return filesystem::is_socket(p); }
        bool is_symlink(const std::string &p) { return filesystem::is_symlink(p); }
