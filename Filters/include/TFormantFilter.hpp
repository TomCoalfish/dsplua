...
template<class T>
inline T MyFilter<T>::processBiquad(T in)
{
	T out = in * a0 + z1;
	z1 = in * a1 + z2 - b1 * out;
	z2 = in * a2 - b2 * out;
...
template<class T>
inline T MyFilter<T>::processButterworth(T in)
{
...
template<class T>
void MyFilter<T>::prepareToPlay(T sampleRate, int samplesPerBlock)
{
...
template<class T>
T MyFilter<T>::simpleFIRHiPass(T currentSample, T sliderValue)
{
...
template<class T>
T MyFilter<T>::simpleIIRLowPass(T currentSample, T sliderValue)
{
...
template<class T>
void MyFilter<T>::setBiquad(int type, T Fc, T Q, T peakGainDB) {
	this->type = type;
	this->Q = Q;
	this->frequency = Fc;
...
template<class T>
void MyFilter<T>::setButterworth(int type, T Fc, T Q, T peakGainDB) {
	this->type = type;
	this->Q = Q;
	this->frequency = Fc;
	this->peakGain = peakGainDB;
...
template<class T>
void MyFilter<T>::calcBiquad(void) {
	T norm;
	T V = pow(10, fabs(peakGain) / 20.0);
...
template<class T>
void MyFilter<T>::calcButterworth(void)
{
	T norm;
...
template<class T>
T MyFilter<T>::simpleIIRHiPass(T currentSample, T sliderValue)
{
	T a0 = sliderValue - 1.0;
